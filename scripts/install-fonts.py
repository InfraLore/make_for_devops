#!/usr/bin/env python3
"""Verify required fonts are installed; download Merriweather if missing.

Reads font requirements from metadata.yml and checks each is available
via fontconfig. Merriweather is downloaded from Google Fonts if absent.

Usage:
    python3 scripts/install-fonts.py              # check and install
    python3 scripts/install-fonts.py --check      # check only (exit 1 if missing)
"""

import glob
import os
import re
import subprocess
import sys
import urllib.request

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
REPO_ROOT = os.path.dirname(SCRIPT_DIR)
METADATA = os.path.join(REPO_ROOT, "metadata.yml")
FONT_DIR = os.path.expanduser("~/.fonts")

MERRIWEATHER_CSS = (
    "https://fonts.googleapis.com/css2?"
    "family=Merriweather:ital,wght@0,300;0,400;0,700;0,900;1,300;1,400;1,700;1,900"
)

WEIGHT_MAP = {
    "300": "Light",
    "400": "Regular",
    "700": "Bold",
    "900": "Black",
}

SUFFIX_MAP = {
    ("Light", True): "LightItalic",
    ("Regular", True): "Italic",
    ("Bold", True): "BoldItalic",
    ("Black", True): "BlackItalic",
}

# Metadata keys that specify fonts
FONT_KEYS = ("mainfont", "monofont", "sansfont")


def read_metadata_fonts():
    """Return dict of {key: font_name} from metadata.yml."""
    fonts = {}
    try:
        with open(METADATA) as f:
            for line in f:
                for key in FONT_KEYS:
                    m = re.match(rf"{key}:\s*(.+)", line)
                    if m:
                        fonts[key] = m.group(1).strip()
    except FileNotFoundError:
        pass
    return fonts


def check_font(family):
    """Return True if font family is found by fontconfig."""
    try:
        result = subprocess.run(
            ["fc-list", "--format=%{family}\n", family],
            capture_output=True, text=True, timeout=10,
        )
    except FileNotFoundError:
        return False
    families = [l.strip() for l in result.stdout.split("\n") if l.strip()]
    for entry in families:
        if family in entry.split(","):
            return True
    return False


def get_font_blocks():
    """Parse Google Fonts CSS for Merriweather @font-face entries."""
    req = urllib.request.Request(
        MERRIWEATHER_CSS, headers={"User-Agent": "Mozilla/5.0"}
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
        css = resp.read().decode("utf-8")

    blocks = re.findall(r"@font-face\s*\{[^}]+\}", css)
    if not blocks:
        print("Error: Could not parse @font-face blocks from Google Fonts CSS")
        sys.exit(1)

    entries = []
    for block in blocks:
        style = re.search(r"font-style:\s*(\w+)", block)
        weight = re.search(r"font-weight:\s*(\d+)", block)
        url = re.search(r"url\(([^)]+)\)", block)
        if style and weight and url:
            entries.append((style.group(1), weight.group(1), url.group(1)))
    return entries


def download_merriweather():
    """Download all Merriweather variants from Google Fonts."""
    print("Downloading Merriweather from Google Fonts...")
    entries = get_font_blocks()

    os.makedirs(FONT_DIR, exist_ok=True)
    downloaded = []
    for style, weight, url in entries:
        is_italic = style == "italic"
        weight_name = WEIGHT_MAP.get(weight, weight)
        suffix = SUFFIX_MAP.get((weight_name, is_italic), weight_name)
        filename = f"Merriweather-{suffix}.ttf"
        dest = os.path.join(FONT_DIR, filename)

        if os.path.exists(dest):
            continue

        print(f"  {filename}...")
        try:
            req = urllib.request.Request(
                url, headers={"User-Agent": "Mozilla/5.0"}
            )
            with urllib.request.urlopen(req, timeout=30) as resp:
                with open(dest, "wb") as f:
                    f.write(resp.read())
            downloaded.append(filename)
        except Exception as e:
            print(f"  Failed: {e}")

    if downloaded:
        subprocess.run(["fc-cache", "-f", FONT_DIR], capture_output=True)
        print(f"  Downloaded {len(downloaded)} file(s)")
    else:
        print("  All variants already present")


def install_instructions(missing):
    """Print platform-specific install commands for missing fonts."""
    print()
    print("Install system fonts via your package manager:")
    print()
    print("  # Ubuntu/Debian:")
    print("  sudo apt install fonts-dejavu-core fonts-lato")
    print()
    print("  # Fedora:")
    print("  sudo dnf install dejavu-fonts-common google-noto-sans-fonts-common")
    print()
    print("  # Arch:")
    print("  sudo pacman -S ttf-dejavu ttf-lato")
    print()
    print("  # macOS (via Homebrew):")
    print("  brew install --cask font-dejavu-sans-mono font-lato")


def main():
    check_only = "--check" in sys.argv

    metadata_fonts = read_metadata_fonts()
    if not metadata_fonts:
        print(f"No fonts found in {METADATA}")
        print("Expected keys: mainfont, monofont, sansfont")
        return 1

    print("Fonts required by metadata.yml:")
    for key, name in sorted(metadata_fonts.items()):
        print(f"  {key}: {name}")
    print()

    # Deduplicate font names (e.g., same font for mainfont and sansfont)
    unique_fonts = sorted(set(metadata_fonts.values()))

    status = {}
    for font in unique_fonts:
        status[font] = check_font(font)

    missing = [f for f, ok in status.items() if not ok]

    # Auto-install Merriweather if missing and not in check-only mode
    if "Merriweather" in missing and not check_only:
        print("Merriweather not found. Installing...")
        download_merriweather()
        status["Merriweather"] = check_font("Merriweather")
        missing = [f for f, ok in status.items() if not ok]

    # Report
    print("Font status:")
    all_ok = True
    for font in unique_fonts:
        ok = status[font]
        mark = "\u2714" if ok else "\u2718"
        print(f"  {mark} {font}")
        if not ok:
            all_ok = False

    if missing:
        print(f"\nMissing: {', '.join(missing)}")
        install_instructions(missing)
        return 1

    print("\nAll required fonts are available.")
    return 0


if __name__ == "__main__":
    sys.exit(main())
