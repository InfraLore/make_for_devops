...
Here's how all these patterns come together:
...
ci: ## Auto-detect and run appropriate pipeline
	@echo "Running $(CI_ENV) pipeline..."
	@$(MAKE) ci-detect-and-run

ci-detect-and-run:
ifeq ($(GITHUB_REF_NAME),main)
	@$(MAKE) ci-main
else ifdef PULL_REQUEST_ID
	@$(MAKE) ci-pr
else
	@$(MAKE) ci-quick
endif

ci-quick: ## Fast validation (< 2 min)
	@$(MAKE) lint test-unit security-quick

ci-pr: ## PR validation (< 10 min)
	@$(MAKE) ci-quick
	@$(MAKE) test-integration build

ci-main: ## Main branch pipeline (< 20 min)
	@$(MAKE) ci-pr
	@$(MAKE) security-deep
	@$(MAKE) build-release
...
deploy-staging: ## Deploy to staging via GitOps
	@$(MAKE) gitops-update ENV=staging

deploy-production: ## Deploy to production
	@echo "Deploying $(VERSION) to production..."
	@$(MAKE) pre-production-checks
	@$(MAKE) gitops-update ENV=production
...
parallel-build: ## Build with parallelization
	@$(MAKE) -j4 build-api build-worker build-frontend

cached-build: ## Build with caching
	@$(MAKE) deps-cache-restore
	@$(MAKE) deps-install
	@$(MAKE) build
...
cache-status: ## Show cache status
	...
	@du -sh $(CACHE_DIR) 2>/dev/null || echo "No cache"

ci-debug: ## Debug CI environment
	@echo "CI Environment: $(CI_ENV)"
	@echo "Version: $(VERSION)"
	@echo "Image: $(IMAGE)"
	@env | grep -E '^(CI|GITHUB|GITLAB)' || true

clean: ## Clean build artifacts
	@rm -rf bin/ artifacts/ $(CACHE_DIR)/
...
metrics-report: ## Show performance trends
	@echo "Last 10 pipelines:"
	@./scripts/show-pipeline-history.sh
```

The goal is continuous improvement. Track how long each stage takes, monitor
cache hit rates, and identify bottlenecks.
...
```makefile
# Troubleshooting
ci-troubleshoot: ## Diagnose common issues
	@echo "Checking common problems..."
	@echo "1. Disk space:"
	@df -h .
	@echo "2. Docker resources:"
	@docker system df
	@echo "3. Cache state:"
	@$(MAKE) cache-status

ci-fix: ## Apply common fixes
	@$(MAKE) clean
